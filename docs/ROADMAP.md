...
- [ ] NCHWc: designed + prototyped (docs/DESIGN-nchwc.md) — recomposing
      existing kernels measured 0.80-0.97× (no win); requires a dedicated
      kernel program (register-tap dw, blocked pointwise, layout pass).
      Prior kills: fused dw+pw 2-3× slower (LLC-resident intermediates),
      pool micro-tuning (3 negatives)
