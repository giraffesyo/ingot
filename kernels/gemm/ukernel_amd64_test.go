...
func TestAVX512MatchesGeneric(t *testing.T) {
	...
	for _, kc := range []int{0, 1, 3, 8, 16, 33, 257} {
		for _, acc := range []bool{false, true} {
			...
			microKernelGeneric(kc, ap, bp, want, ldc, acc)
			microKernelAVX512(kc, ap, bp, got, ldc, acc)
			for i := range want {
				if math.Abs(float64(want[i]-got[i])) > 1e-4*(1+math.Abs(float64(want[i]))) {
					t.Fatalf("kc=%d acc=%v idx %d: want %g got %g", kc, acc, i, want[i], got[i])
				}
			}
		}
	}
...
func BenchmarkMicroKernelVariants(b *testing.B) {
	...
	bench := func(name string, fn func(int, []float32, []float32, []float32, int, bool)) {
		b.Run(name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				fn(kc, ap, bp, c, NR, true)
			}
